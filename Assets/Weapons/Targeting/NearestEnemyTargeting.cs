...
public class NearestEnemyTargeting : TargetingStrategy
{
    ...
    public override targetInfo GetTargets(WeaponContext context)
    {
        ...
        Collider2D[] hits =
            Physics2D.OverlapCircleAll(pos, maxDistance, enemyMask);
        ...
        float bestDist =
            ((Vector2)best.transform.position - pos).sqrMagnitude;

        for (int i = 1; i < hits.Length; i++)
        {
            float d =
                ((Vector2)hits[i].transform.position - pos).sqrMagnitude;
        ...
        Vector3 dir =
            ((Vector2)best.transform.position - pos).normalized;
